...
# 1. Load the target clinical features (Ignoring any Unnamed placeholder lines)
with open("redcap_tokeep.txt", "r", encoding="utf-8") as f:
    columns_to_keep = [
        line.strip() for line in f
        if line.strip() and "Unnamed" not in line
    ]

csv_filename = "/jbod2/def-gimap5/20250923_BQC19/data/clinical/redcap_clinical_data_label_2024-04-11.csv"

# Read header removing BOM characters cleanly
...
final_filename = "redcap.filtered_output.tsv"  # Changed to .tsv extension
...
for chunk in pd.read_csv(csv_filename, usecols=valid_columns, chunksize=chunk_size, low_memory=False, engine='c',
                         quoting=csv.QUOTE_MINIMAL):
    chunk = chunk[valid_columns]
    if id_col in chunk.columns:
        chunk = chunk.rename(columns={id_col: "BQC ID"})

    # --- WRITE INTERMEDIATE AS TAB SEPARATED ---
    chunk.to_csv(temp_filename, mode='a', sep="\t", index=False, header=first_chunk)
    first_chunk = False

# 5. Load extracted data and re-evaluate conflicts safely
print("Analyzing dataset for longitudinal value conflicts...")
raw_extracted_df = pd.read_csv(temp_filename, sep="\t", low_memory=False)
...
metrics_flattened = raw_extracted_df.groupby("BQC ID", as_index=False)[
    ["BQC ID"] + [c for c in max_value_columns if c in raw_extracted_df.columns]].max()
...
# --- WRITE FINAL MATRIX TO TSV DELIMITER ---
flattened_df.to_csv(final_filename, sep="\t", index=False)
